 ...
 **       write that appears to complete successfully but is in fact truncated.  This
 **       is only true for the first file to pass EWEOM; subsequent requests to create
 **       a file will fail when the create request is processed.
 **
 *************************************************************************************
 **
	...
	bool ip_state, struct write_request *req, struct dentry *d, struct unified_data *priv)
{
	...
	*cache = NULL;
	...
	/* If the cache size is 64k then the bytes passed to ltfs process (<=128k each)
	 * may be more than 64k.In this case we need to return actual copy_count(max 64k) to
	 * consider the remaining bytes in the next iteration.
	 */
	if (count > priv->cache_size)
...
int _unified_flush_unlocked(struct dentry *d, struct unified_data *priv)
{
     ...
     * OSR
     *
     * We're seeing this entry left on the DP queue while running 
     * one of our tests. It's then freed, but the leftover entry on
     * the queue results in a use after free crash.
     *
...
struct iosched_ops *iosched_get_ops(void)
{
	...
	int ret = 0;
	ios_profiler = NULL;
	ret = ltfs_mutex_init(&ios_profiler_lock);
	if (ret) {
